<html>
	<head>
		...
		<body>
			...
			<!-- header -->
			<div class="header" align="center"><h1><span style="font-family: &quot;Comic Sans MS&quot;;">AES-192</span></h1></div>
			<!-- formularz -->
		...
		if(isset($_POST['add']))
		{
			...
			if ($mode == 'CBC')
			{
			$encrypted_openssl = openssl_encrypt($data, "AES-192-CBC", 
				$key, OPENSSL_RAW_DATA | OPENSSL_ZERO_PADDING, $iv);
			}
			else if ($mode == 'ECB')
			{
			$encrypted_openssl = openssl_encrypt($data, "AES-192-ECB", 
				$key, OPENSSL_RAW_DATA | OPENSSL_ZERO_PADDING);
			}
			else if ($mode == 'CFB')
			{
			$encrypted_openssl = openssl_encrypt($message, "AES-192-CFB", 
				$key, OPENSSL_RAW_DATA  , $iv);
			}
			else if ($mode == 'OFB')
			{
			$encrypted_openssl = openssl_encrypt($message, "AES-192-OFB", 
									...
									<div class="container-fluid h-100">
											<div class="row justify-content-center align-items-center h-100">
												<div class="col col-sm-12 ">
													...
													<p style="text-align: justify;">AES w wersji 192-bitowej przetwarza 128-bitowe bloki przy użyciu klucza wynoszącego  192-bit&oacute;w. Podczas szyfrowania stosowane są operacje przesuwania, podstawiania i zamiany bit&oacute;w oraz operacje XOR. AES ma zmienną liczbę rund &ndash; jest ona uzależniona od długości klucza i w tej wersji wynosi ona 12.</p>
