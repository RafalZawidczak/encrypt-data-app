<html>
	<head>
		...
		<title>Camellia-256</title>
	</head>
		<body>
			<!-- NAVBAR -->
			<?php include 'src/navbar.php'; ?>
			<!-- header -->
			<div class="header" align="center"><h1><span style="font-family: &quot;Comic Sans MS&quot;;">Camellia-256</span></h1></div>
			<!-- formularz -->
		...
		if(isset($_POST['add']))
		{
			...
			if ($mode == 'CBC')
			{
			$encrypted_openssl = openssl_encrypt($data, "Camelia-256-CBC", 
				$key, OPENSSL_RAW_DATA | OPENSSL_ZERO_PADDING, $iv);
			}
			else if ($mode == 'ECB')
			{
			$encrypted_openssl = openssl_encrypt($data, "Camelia-256-ECB", 
				$key, OPENSSL_RAW_DATA | OPENSSL_ZERO_PADDING);
			}
			else if ($mode == 'CFB')
			{
			$encrypted_openssl = openssl_encrypt($message, "Camelia-256-CFB", 
				$key, OPENSSL_RAW_DATA  , $iv);
			}
			else if ($mode == 'OFB')
			{
			$encrypted_openssl = openssl_encrypt($message, "Camelia-256-OFB", 
									...
									<div class="container-fluid h-100">
											<div class="row justify-content-center align-items-center h-100">
												<div class="col col-sm-11 ">
													...
													<p style="text-align: justify;">Camellia w wersji 256-bitowej pracuje na kluczu o długości 256 bit&oacute;w. Wielkość bloku tekstu jawnego i szyfrogramu wynosi&nbsp; 128 bit&oacute;w.</p>
													...
													<p style="text-align: justify;">W wersji 256-bitowej wykonywane są 4 powt&oacute;rzenia blok&oacute;w sześciu rund.</p>
